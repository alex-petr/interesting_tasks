class Parser
  ...
  def parse_category(donor_category)
    ...
    while next_page
      page_url = "#{donor_category.url}#{paginate_substring(page_num)}"
      ...
      if doc.class.to_s == 'TrueClass'
        category_logger.donor_category_pages_log.warn("#{Time.zone.now} TrueClass returned instead of page #{page_url}")
        next
      end
